...
FIELDNAMES = ['NoPol', 'NamaPemilik', 'MerkMobil', 'MasaBerlaku', 'BahanBakar', 'Warna']
...
def lihat_data():
    ...
    for i, row in enumerate(data, start=1):
        print(f"{i}. {row['NoPol']} - {row['NamaPemilik']} - {row['MerkMobil']} - Berlaku sampai: {row['MasaBerlaku']} - BBM: {row['BahanBakar']} - Warna: {row['Warna']}")
...
def edit_data():
    ...
    for row in data:
        if row['NoPol'] == nopol:
            ...
            row['Warna'] = input("Warna Mobil baru: ")
            save_data(data)
